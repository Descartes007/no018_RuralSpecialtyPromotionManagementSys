...
<!--          </el-button>-->
          ...
          <el-button type="success" size="mini" @click="updateInventory(row)">
            修改库存
          </el-button>
...
import {listAll,deleteGoods,updateInventory} from '@/api/goods'
import waves from '@/directive/waves' // waves directive
import { parseTime } from '@/utils'
import Pagination from '@/components/Pagination'
import {updatePassword} from "@/api/user"; // secondary package based on el-pagination
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listAll(this.listQuery).then(response => {
        if(this.listQuery.title){
          this.list = response.data.filter(v=>v.title.includes(this.listQuery.title))
        }else{
    ...
    updateInventory(row){
      this.$prompt('请输入新的库存量', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
      }).then(async ({ value }) => {
        await updateInventory({id:row.id,inventory:value})
        this.getList()
      }).catch(() => {
        this.$message({
          type: 'info',
