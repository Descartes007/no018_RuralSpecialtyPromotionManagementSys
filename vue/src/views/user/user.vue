...
<!--      </el-checkbox>-->
    ...
    >
      ...
      <el-table-column label="身份">
        <template slot-scope="{row}">
          <span>{{ row.role | statusFilter}}</span>
        </template>
      </el-table-column>
      <el-table-column label="头像">
        <template slot-scope="{row}">
          <template v-if="row.avatar">
            <a target="_blank" :href="row.avatar">查看头像</a>
          </template>
          <template v-else>
            未上传头像
          </template>
...
<!--          </el-button>-->
    ...
    <el-dialog :visible.sync="contentDialogPvVisible" title="修改密码">
...
import {auditReleaseNode, auditDefeatNode, deleteNode} from '@/api/node'
import {list as listAll,deleteUser,updatePassword} from '@/api/user'
  ...
  methods: {
    showContent(row){
      this.$prompt('请输入密码', '提示', {
        ...
        await updatePassword({userId:row.id,oldPassword:row.password,newPassword:value})
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '取消输入'
    ...
    getList() {
      ...
      listAll(this.listQuery).then(response => {
        if(this.listQuery.title){
          this.list = response.data.filter(v=>v.name.includes(this.listQuery.title))
        }else{
