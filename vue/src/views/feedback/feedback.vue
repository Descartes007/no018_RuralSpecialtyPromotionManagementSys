...
<!--            {{ $t('table.edit') }}-->
<!--          </el-button>-->
          <el-button v-if="!row.replyContent" type="primary" size="mini" @click="showContent(row)">
            回复
          </el-button>
  ...
  methods: {
    showContent(row){
      this.$prompt('请输入回复内容', '提示', {
        ...
        await updateFeedback({id:row.id,replyContent:value})
        this.getList()
      }).catch(() => {
        this.$message({
          type: 'info',
    ...
    getList() {
      ...
      listFeedback(this.listQuery).then(response => {
        if(this.listQuery.title){
          this.list = response.data.filter(v=>v.content.includes(this.listQuery.title))
        }else{
